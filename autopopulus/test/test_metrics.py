...
class TestMetrics(unittest.TestCase):
    ...
    def test_static_onehot_categorical(self, df):
        ...
        with self.subTest("No Mask"):
            ...
            with self.subTest("Not Equal"):
                ...
                self.assertAlmostEqual(
                    # 1 error for 1 feature out of F
                    (((N - 1) / N) + F - 1) / F,
    ...
    def test_static_multicat_continuous(self, df):
        ...
        tensor_df = torch.tensor(df.values)
        with self.subTest("No Mask"):
            self.assertAlmostEqual(0, RMSE(tensor_df, tensor_df).item(), places=WITHIN)
            self.assertAlmostEqual(
                0,
                MAAPE(tensor_df, tensor_df).item(),
                places=WITHIN,
            )
        ...
        with self.subTest("Mask"):
            ...
            self.assertAlmostEqual(
                0,
                RMSE(tensor_df, tensor_df, missing_mask).item(),
                places=WITHIN,
            )
            self.assertAlmostEqual(
                0,
                MAAPE(tensor_df, tensor_df, missing_mask).item(),
                places=WITHIN,
            )
            ...
            with self.subTest("Not Equal inside mask"):
                ...
                self.assertAlmostEqual(
                    0,
                    RMSE(error_df, tensor_df, missing_mask).item(),
                    places=WITHIN,
                )
                self.assertAlmostEqual(
                    0,
                    MAAPE(error_df, tensor_df, missing_mask).item(),
                    places=WITHIN,
                )
